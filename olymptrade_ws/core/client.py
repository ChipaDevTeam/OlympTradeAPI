...
class OlympTradeClient:
    def __init__(self, access_token: str, uri: str = settings.DEFAULT_WEBSOCKET_URI, log_raw_messages: bool = False):
    ...
    async def send_request(self, event_code: int, data: Any, requires_response: bool = True, timeout: Optional[float] = None) -> Optional[Dict[str, Any]]:
        ...
        if future:
            try:
                response_timeout = timeout if timeout is not None else settings.DEFAULT_RESPONSE_TIMEOUT
